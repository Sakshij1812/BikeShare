...
def windowInitialization():
    ...
    window = Tk()
    window.title("Welcome to Next Bike App")
    ...
    #station table frame
    station_frame = Frame(window, width = 300, height = 300)
    ...
    #bike table frame
    bike_frame = Frame(window)
    bike_scroll = ttk.Scrollbar(bike_frame)
    bike_tree = ttk.Treeview(bike_frame, yscrollcommand=bike_scroll.set, selectmode = "extended")
    
    
    #rent button
    rent_button = Button(window, text="Click to Start Rental", 
                 command = startBikeRental, 
                 bg = 'white', fg = 'blue', 
                 height = 2, width = 15)
    
    rent_button['font'] = myFont
    
    #rental status frame
    rental_status_frame = LabelFrame(window, text = "Rental Status")
    
    #return station frame
    return_station_frame = Frame(window, width = 260, height = 300)
    ...
    #return buttons
    return_button = Button(rental_status_frame, text="Click to Return Bike", command = showReturnStationList, 
                           height = 4, width = 25)
    return_button['font'] = myFont
    
    return_button2 = Button(rental_status_frame, text="Select a station to return ⏩",  
                           height = 4, width = 25)
    return_button2['font'] = myFont
    
    return_button3 = Button(rental_status_frame, text="Click to confirm return", command = returnBike,
                            bg = 'white', fg = 'blue', 
                           height = 4, width = 25)
    return_button3['font'] = myFont
    
    
    #different label frames and corresponding labels
    
    #TODO - Replace hard coded user name with user name from session
    customer_name = "K Macmillan"
    ...
    greeting_label_frame = LabelFrame(window, text = "Hey "+customer_name+", ")
    
    now = datetime.datetime.now()
    
    wallet_balance = getWalletBalace()
    ...
    greeting_label_frame2 = LabelFrame(window, text = "Hey "+customer_name+", ")
    greeting_label2 = Label(greeting_label_frame2, text="\nEnjoy your ride! 🚴‍♀️")
    
    bike_list_label_frame_a = LabelFrame(window, text = "Bikes are available 😀")
    bike_list_label_a = Label(bike_list_label_frame_a, text="\nSelect a bike below and start your rental 👍")
    
    bike_list_label_frame_na = LabelFrame(window, text = "Bikes are not available 🙁")
    ...
    rental_progress_frame = LabelFrame(window, text = "Rental is in progress")
    ...
    low_balance_frame = LabelFrame(window, text = "Low Wallet Balance 🙁")
...
#    print("inside getStationList")
    cursor.execute("""SELECT station_name, 
    case 
...
#    print("inside getActiveRental")
    ...
    cursor.execute("""select b.type, b.bike_number, s.station_name, s.latitude , s.longitude, 
    ...
    ;""", (customer_id, ))
    active_rental = cursor.fetchall()
#    print("active rental count: ",len(active_rental))
    return active_rental

#function to populate station list to station tree
def populateStationList(station_list):
    #station table frame
    station_frame.place(x = 10, y = 100, width = 720, height = 240)
    station_scroll.pack(side=RIGHT, fill=Y)
    station_tree.pack()
    station_scroll.config(command = station_tree.yview)
    station_tree['columns'] = ("Station", "Bikes", "Racks")
    station_tree.column("#0", width = 0, stretch = NO)
    station_tree.column("Station", anchor=W, width=240)
    station_tree.column("Bikes", anchor=CENTER, width=240)
    station_tree.column("Racks", anchor=CENTER, width=240)
    station_tree.heading("#0",text = "", anchor = W)
    station_tree.heading("Station",text = "Station", anchor = W)
    station_tree.heading("Bikes",text = "Bikes", anchor = W)
    station_tree.heading("Racks",text = "Racks", anchor = W)
    station_tree.tag_configure('evenrow', background = '#E2F0CB')
    station_tree.tag_configure('oddrow', background = '#B5EAD7')
...
#        print("record: ", record)
        if count % 2 == 0:
            station_tree.insert(parent='', index = 'end', iid = count, text = '', values = (record[0], record[1], record[2], record[3]), tags=('evenrow',))
        else:
            station_tree.insert(parent='', index = 'end', iid = count, text = '', values = (record[0], record[1], record[2], record[3]), tags=('oddrow',))
        count +=1
...
#    print("inside selectStation")
    ...
    #clear bike tree
    clearBikeTree()
    bike_frame.place_forget()
    hideLowWalletLabel()
    #hide rent button
    rent_button.place_forget()
    selected = station_tree.focus()
    values = station_tree.item(selected, 'values')
...
#    print("selectStation: ",values[3])
    ...
    cursor.execute(selected_station_query,(values[3],))
    bike_list = cursor.fetchall()
#    print("bike_list: ")
#    print(bike_list)
#    print("length of bike list: ",len(bike_list))
    active_rental = getActiveRental()
#    print("length of active rental: ",len(active_rental))
    ...
    #    print("inside first if")
        if len(bike_list) > 0:
            bikeAvailableLabel()
            populateBikeList(bike_list)
        #    print("to populate bike list")
        else:
            bikeNotAvailableLabel()
    else:
        print(active_rental)

    
#function to populate bike list in bike_tree        
def populateBikeList(bike_list):
    bike_frame.place(x = 10, y = 430, width = 720, height = 240)
    bike_scroll.pack(side=RIGHT, fill=Y)
    bike_tree.pack()
    bike_scroll.config(command = bike_tree.yview)
    bike_tree['columns'] = ("Station","Type", "BikeNum")
    bike_tree.column("#0", width = 0, stretch = NO)
    bike_tree.column("Station", anchor=W, width=240)
    bike_tree.column("Type", anchor=CENTER, width=240)
    bike_tree.column("BikeNum", anchor=CENTER, width=240)
    bike_tree.heading("#0",text = "", anchor = W)
    bike_tree.heading("Station",text = "Station", anchor = W)
    bike_tree.heading("Type",text = "Type", anchor = W)
    bike_tree.heading("BikeNum",text = "BikeNum", anchor = W)
    bike_tree.tag_configure('evenrow', background = '#E2F0CB')
    bike_tree.tag_configure('oddrow', background = '#B5EAD7')
    
    #populate bike records from db to bike table frame
    count = 0
    for record in bike_list:
        if count % 2 == 0:
            bike_tree.insert(parent='', index = 'end', iid = count, text = '', values = (record[0], record[1], record[2], record[3], record[4]), tags=('evenrow',))
        else:
            bike_tree.insert(parent='', index = 'end', iid = count, text = '', values = (record[0], record[1], record[2], record[3], record[4]), tags=('oddrow',))
        count +=1
...
#    print("inside selectBike")
    selected = bike_tree.focus()
    values = bike_tree.item(selected, 'values')
#    print("selected_bike: ", values)
    ...
    #    print("balance less than -10")
        ...
        rent_button.place(x = 800, y = 300, width = 400, height = 200)
...
#    print("inside getWalletBalace")
    ...
    get_wallet_balance_query = """select wallet_balance
                   ...
                   limit 1;"""
    cursor.execute(get_wallet_balance_query, (customer_id, ))

    wallet_balance = cursor.fetchall()
...
#    print("selected bike: ",values)
    ...
    #customer_id from session
    cursor.execute(insert_rental_query, (customer_id, values[4], values[3]))
    
    update_bike_status_query="""update bike_status set is_rented = 1, is_available = 0 where bike_id = ?"""
    cursor.execute(update_bike_status_query, (values[4],))
    ...
    populateStationList(station_list)
    bike_list = []
    clearBikeTree()
    bike_tree.pack_forget()
...
#    #    print("inside if len activerental > 0")
        populateRentalStatus(active_rental)
        enjoyRideLabel()
    else:
#    #    print("inside else")
        goodMorningLabel()
...
#    print("inside enjoyRideLabel")
    bike_list_label_a.grid_forget() 
    bike_list_label_frame_a.place_forget()
    bike_list_label_na.grid_forget() 
    bike_list_label_frame_na.place_forget()
    greeting_label.grid_forget()
    greeting_label_frame.place_forget()
    greeting_label_frame2.place(x=10, y=10, width = 720, height = 90) 
    greeting_label2.grid(column=0, row=0, sticky=W, padx=5, pady=5)
    
#functino to greet user    
def goodMorningLabel():
#    print("inside goodMorningLabel")
    greeting_label_frame2.place_forget()
    greeting_label2.grid_forget()
    ...
    greeting_label_frame.place(x=10, y=10, width = 720, height = 90) 
    greeting_label.grid(column=0, row=0, sticky=W, padx=5, pady=5)
    

#function to show labels when bikes are available in a station
def bikeAvailableLabel():
#    print("inside bikeAvailableLabel")
    bike_list_label_frame_na.place_forget()
    bike_list_label_na.grid_forget()
    bike_list_label_frame_a.place(x=10, y=340, width = 720, height = 90)
    bike_list_label_a.grid(column=0, row=0, sticky=W, padx=5, pady=5)
    
#function to show label when bikes are not available in a station    
def bikeNotAvailableLabel():
#    print("inside bikeNotAvailableLabel")
    bike_list_label_frame_a.place_forget()
    bike_list_label_a.grid_forget()
    bike_list_label_frame_na.place(x=10, y=340, width = 720, height = 90)
    bike_list_label_na.grid(column=0, row=0, sticky=W, padx=5, pady=5)
...
#    low_balance_frame.place(x=730, y=450, width = 680, height = 90)
    low_balance_frame.place(x = 10, y = 430, width = 720, height = 240)
    low_balance_label.grid(column=0, row=0, sticky=W, padx=5, pady=15)   

#function to hide label for low amount in wallet
def hideLowWalletLabel():
#    print("inside lowWalletLabel")
    low_balance_frame.place_forget()
    low_balance_label.grid_forget()

#function to clear bike_bike tree
def clearBikeTree():
    #Clear the treeview list items
    for item in bike_tree.get_children():
        bike_tree.delete(item)
    
#function to clear station tree
def clearStationTree():
    #Clear the treeview list items
    for item in station_tree.get_children():
        station_tree.delete(item)
    
#function to clear return station tree
def clearReturnStationTree():
    #Clear the treeview list items
    for item in return_station_tree.get_children():
        return_station_tree.delete(item)
    

#function to populate active rental status        
def populateRentalStatus(active_rental):
#    print("inside populateRentalStatus")
#    print(active_rental[0])
#    print(active_rental[0][0])
    
    
    rental_progress_frame.place(x=730, y=10, width = 690, height = 90)
    rental_progress_label.grid(column=0, row=0, sticky=W, padx=5, pady=5)
    
    rental_status_frame.place(x=730, y=100, width = 690, height = 300)  

    #bike type
    bike_type_label = Label(rental_status_frame, text="Bike type:")
    bike_type_label.grid(column=0, row=0, sticky=W, padx=5, pady=5)
    
    bike_type_entry = Label(rental_status_frame, text = active_rental[0][0])
    bike_type_entry.grid(column=1, row=0, sticky=E, padx=5, pady=5)
    
    # bike num
    bike_number_label = Label(rental_status_frame, text="Bike number:")
    bike_number_label.grid(column=0, row=1, sticky=W, padx=5, pady=5)
    
    bike_number_entry = Label(rental_status_frame, text = active_rental[0][1])
    bike_number_entry.grid(column=1, row=1, sticky=E, padx=5, pady=5)
    
    
    # start station
    from_station_label = Label(rental_status_frame, text="Started from:")
    from_station_label.grid(column=0, row=2, sticky=W, padx=5, pady=5)
    
    from_station_entry["text"] = active_rental[0][2]
    from_station_entry.grid(column=1, row=2, sticky=E, padx=5, pady=5)
    
    # start time
    from_time_label = Label(rental_status_frame, text="Started at:")
    from_time_label.grid(column=0, row=3, sticky=W, padx=5, pady=5)
    
    from_time_entry = Label(rental_status_frame, text = active_rental[0][5])
    from_time_entry.grid(column=1, row=3, sticky=E, padx=5, pady=5)
    
    # rent status
    rent_status_label = Label(rental_status_frame, text="Rental status:")
    rent_status_label.grid(column=0, row=4, sticky=W, padx=5, pady=5)
    
    rent_status_entry = Label(rental_status_frame, text = active_rental[0][6])
    rent_status_entry.grid(column=1, row=4, sticky=E, padx=5, pady=5)
    
    # return button
    return_button.grid(column=1, row=7, sticky=E, padx=5, pady=5)
    

#function to show station list for return
def showReturnStationList():
#    print("inside showReturnStationList")
    return_button2.grid(column=1, row=7, sticky=E, padx=5, pady=5)
    station_list = []
    station_list = getStationList()
#    clearStationTree()
#    station_frame.place_forget()
    populateReturnStationList(station_list)


#function to populate return station list to return station tree
def populateReturnStationList(station_list):
    #return station table frame
#    return_station_frame.place(x = 900, y = 400, width = 300, height = 240)
    return_station_frame.place( x = 1110, y = 130, width = 300, height = 240)
#    return_station_scroll.pack(side=RIGHT, fill=Y)
    return_station_tree.pack()
#    return_station_scroll.config(command = return_station_tree.yview)
    return_station_tree['columns'] = ("Station")
    return_station_tree.column("#0", width = 1, stretch = NO)
    return_station_tree.column("Station", anchor=CENTER, width=300)
    return_station_tree.heading("#0",text = "", anchor = W)
    return_station_tree.heading("Station",text = "Select return station", anchor = W)
    return_station_tree.tag_configure('evenrow', background = 'ivory2')
    return_station_tree.tag_configure('oddrow', background = 'ivory3')
    ...
    #    print("record: ", record)
        if count % 2 == 0:
            return_station_tree.insert(parent='', index = 'end', iid = count, text = '', values = (record[0], record[1], record[2], record[3]), tags=('evenrow',))
        else:
            return_station_tree.insert(parent='', index = 'end', iid = count, text = '', values = (record[0], record[1], record[2], record[3]), tags=('oddrow',))
        count +=1
        
#function to call when return station is selected from the list
def selectReturnStation(e):
#    print("inside selectReturnStation")
    selected = return_station_tree.focus()
    values = return_station_tree.item(selected, 'values')
#    print("selectStation: ",values)
    return_button3.grid(column=1, row=7, sticky=E, padx=5, pady=5)
...
#    print("stationID:", values)
    ...
    #customer_id from session
    cursor.execute("""SELECT id, bike_id, start_station_id from rental
                   where customer_id = ? and lower(rental_status) = 'ongoing'
                   order by id desc
                   limit 1;""", (customer_id, ))
    ongoing_ride = cursor.fetchall()
...
#    print(ongoing_ride[0])
    ...
    where bike_id = ?
    """
    cursor.execute(return_bike_query_bike_status, (values[3], ongoing_ride[0][1], ) )
    db.commit()
    
    return_bike_query_rental_endtime ="""update rental set end_time = datetime('now', 'localtime')
    where id = ?"""
    cursor.execute(return_bike_query_rental_endtime, (ongoing_ride[0][0], ))
    db.commit()
    ...
    where id = ?"""
    cursor.execute(return_bike_query_rental, (values[3], ongoing_ride[0][0],  ) )
    db.commit()
    ...
    #customer_id from session
    cursor.execute(return_bike_query_payment, (ongoing_ride[0][0], customer_id, ) )
    db.commit()
    
    return_bike_query_paid ="""update rental set 
    payment_status = 'paid' 
    where id = ? and payment_status = 'calculated'"""
    cursor.execute(return_bike_query_paid, (ongoing_ride[0][0], ) )
    db.commit()
    
    rental_status_frame.place_forget()
    rental_progress_frame.place_forget()
    rental_progress_label.grid_forget()
    from_station_entry.grid_forget()
    
    station_list = []
    station_list = getStationList()
    clearStationTree()
    station_frame.place_forget()
    return_station_frame.place_forget()
    populateStationList(station_list)
    goodMorningLabel()
    return_button.grid_forget()
    return_button2.grid_forget()
    return_button3.grid_forget()
    clearReturnStationTree()
#    print("after return")
    openRideSummary()
     
def openRideSummary():
     
    rideSummary = Toplevel(window)
    ...
    feedback_button = Button(rideSummary, text="Submit Feedback",
                       command=lambda: [updateFeedback(feedback_entry.get(), last_rental[0][0], rideSummary)], height = 4, width = 28).place(x = 350, y = 180 )
    
 
#function to update feedback in the database 
def updateFeedback(feedback_entry, rental_id, rideSummary):
...
#    print(rental_id)
    ...
    set feedback  = ? 
    where id = ?  """
    cursor.execute(update_feedback_query, (feedback_entry,rental_id,) )
    db.commit()
...
#    print("inside getLastRental")
    ...
    #customer_id from session
    cursor.execute("""select r.id, b.type, b.bike_number, s.station_name start_station,
    strftime('%H:%M on %d-', r.start_time)||
    substr('JanFebMarAprMayJunJulAugSepOctNovDec', 1 + 3*strftime('%m', date(r.start_time)), -3) as start_time,
...
window, station_frame, station_scroll, station_tree, bike_frame, bike_scroll, bike_tree, return_station_frame, return_station_scroll, return_station_tree, style, rent_button, rental_status_frame, return_button, return_button2, return_button3, greeting_label_frame, greeting_label, greeting_label_frame2, greeting_label2, bike_list_label_frame_a, bike_list_label_a, bike_list_label_frame_na, bike_list_label_na, rental_progress_frame, rental_progress_label, from_station_entry, low_balance_frame, low_balance_label = windowInitialization()
...
station_tree.bind("<ButtonRelease-1>", selectStation)  

#call selectBike function upon click on bike record
bike_tree.bind("<ButtonRelease-1>", selectBike)

#call selectReturnStation upon click on station to return
return_station_tree.bind("<ButtonRelease-1>", selectReturnStation)
